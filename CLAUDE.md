...
- Phase 4a (JNI host units) is done:
  `docs/superpowers/plans/2026-09-14-phase4a-jni-host-units.md`.
- Phase 4b (library-mode guest process) is done:
  `docs/superpowers/plans/2026-09-14-phase4b-library-runtime.md`. `zbhost` runs as a
...
- Phase 4d Tasks 1-7 are done: ELF preparation and symbol scanning, `Java_*` binding, the
...
- `process`: the main loop, the reusable stop dispatcher (`dispatch_stop`/`after_stop`),
  and host-call/borrower JIT plumbing (Phase 4b).
- `jni_mangle`, `jni_shorty`, `native_call`, `jni_handles`, `native_thunks`
  (`core/src/jni/`): host units of the JNI bridge (Phase 4a): export name decoding,
  shorties, AAPCS32 marshaling, 32-bit handles, and the precompiled arm64 thunk pool.
- `library_protocol`: the C-compatible `zbhost` handshake ABI and guest `ZB_GUEST_RTLD_*`
  constants (Phase 4b).
...
- **Host glibc `RTLD_*` values differ from arm32 bionic's.** Guest loader calls
  (`load_library`, `find_symbol`) take `ZB_GUEST_RTLD_*` (`library_protocol.h`), never
  host `<dlfcn.h>` constants. Guest `RTLD_DEFAULT` is `0xffffffff`.
- **`LibraryRuntime` is process-lifetime.** There is no shutdown path; destroying a
  runtime after `start()` succeeded is a fatal error (it aborts).
- **A borrower costs two processor ids and two JITs** (the carrier and the borrower).
  Carrier JITs, which only run thread start-up and parking, use a 2 MiB code cache
  instead of the default 32 MiB.
- **PI futexes are unsupported on borrowers.** The kernel records the calling host tid
  as the owner, not the guest tid bionic stored in the mutex; the syscall layer refuses
